...
function woocommerce_table_rate_shipping_init() {
			...
			class BE_Table_Rate_Shipping extends WC_Shipping_Method {
				...
				*/
				function init() {
					...
					$this->highest_costing_class = $this->settings['highest_costing_class'];
					$this->no_fee_free_ship = $this->settings['no_fee_free_ship'];
					$this->round_weight = $this->settings['round_weight'];
					$this->hide_method = $this->settings['hide_method'];
					$this->cost_max = $this->settings['cost_max'];
				...
				function init_form_fields() {
					...
					$this->form_fields = array(
						...
						'round_weight' => array(
							...
							'type' => 'checkbox',
							'label' => __( 'Rounds weight value up to the next whole number', 'be-table-ship' ) . ' <b>' . __( '(Per Class Method Only)', 'be-table-ship' ) . '</b>',
							'default' => 'no',
							),
						'hide_method' => array(
			    	...
			    	if(count($rate) > 0) {
			    		...
			    		foreach ($properOrderedArray as $value) {
			    			if(is_array($value)) {
			    				...
			    				if( $this->no_fee_free_ship != 'yes' || ( $this->no_fee_free_ship == 'yes' && $value['cost'] > 0 ) ) {
						    		if( isset($handlingPer) ) $value['cost'] += ( (float) $handlingPer / 100) * $cart_subtotal;
						    		if( isset($handlingFee) ) $value['cost'] += (float) $handlingFee;
						    	}
				...
				function calculate_shipping_perclass( $data = array() ) {
		    		...
		    		for( $i=1; $i<count($shippingTitles);$i++) {
		    			$differences = array();
		    			if( is_array( $compArray ) && is_array( $shippingTitles[$productClasses[ $i ]] ) ) {
	...
	?>
		            	...
		            	<table class="shippingrows widefat" cellspacing="0">
		            		<thead>
		            			<tr>
		        	            	...
		        	            	<th><?php _e( 'Cost', 'be-table-ship' ); ?> <a class="tips" data-tip="<?php echo $cur_symbol; _e(' - Fixed Price&lt;br /&gt;% - Percentage of Cart Subtotal&lt;br /&gt;x - Multiply cost by quantity&lt;br /&gt;(does not apply to Per Item method)&lt;br /&gt;w - Multiply cost by weight&lt;br /&gt;D - Deny: the titled shipping rate will be removed','be-table-ship'); ?>">[?]</a></th>
		    	...
		    	<tr valign="top" id="shipping_class_priorities">
		            ...
		            <td class="forminp" id="<?php echo $this->id; ?>_class_priorities">
		            	<table class="shippingrows widefat" cellspacing="0">
		            		...
		            		<tfoot>
		            			<tr>
		            				<th colspan="3"><i><?php _e( 'These priorities will be used to calculate the appropriate "Per Order" shipping price in the table above. When an order has items of different shipping classes, the one with the highest priority will be used.', 'be-table-ship' ); ?></i></th>
		            			</tr>
		            		</tfoot>
		            		<tbody class="class_priorities">
		                	<?php
			...
			<h4 class="title_drop title_h4 ship_free_title"><?php _e('Set the Order Shipping Options Will Appear','be-table-ship'); ?></h4>
			<table class="form-table">
...
?>
	...
	<?php
				 ...
				 * @return string
				 */
				function sort_table_rates( $table_rates = array() ) {
					// Obtain a list of columns
