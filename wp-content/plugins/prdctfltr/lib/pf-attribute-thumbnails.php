<?php
/**
 * Product Filter add attributes thumbnails
 */

class PF_Attribute_Images_Init {
	...
	public function __construct() {
		...
		add_action('wp_enqueue_scripts', array(&$this, 'pf_enqueue_scripts'));
		add_action('admin_head', array(&$this, 'pf_enqueue_scripts'));

		$this->pf_images = new PF_Attribute_Images();
	...
	public function pf_enqueue_scripts() {
		...
		if (is_admin() && ( $pagenow == 'post-new.php' || $pagenow == 'post.php' || $pagenow == 'edit.php' || 'edit-tags.php')) {
			...
			//wp_enqueue_script('swatches-and-photos-admin', $this->plugin_url() . 'pf-attribute-thumbnails.js', array('jquery'), '1.0', true);
		}
	}
...
$GLOBALS['woocommerce_product_filter'] = new PF_Attribute_Images_Init();
...
class PF_Attribute_Images {
	...
	public function pf_add_attribute_image() {
		global $woocommerce;
		?>
		<div class="form-field swatch-field swatch-field-photo" style="overflow:visible;">
			<div id="swatch-photo" class="<?php echo sanitize_title($this->pf_meta); ?>-photo">
	...
	public function pf_edit_attribute_image($term, $taxonomy) {
		...
		$swatch_term = new PF_Attribute_Image($this->pf_meta, $term->term_id, $taxonomy, false, $this->image_size);
		$image = '';
		?>
		<tr class="form-field swatch-field swatch-field-photo" style="overflow:visible;">
			<th scope="row" valign="top"><label><?php _e('Photo', 'prdctfltr'); ?></label></th>
			<td>
				<div id="pf_thumbnail_<?php echo $this->pf_meta; ?>" style="float:left;margin-right:10px;">
					<img src="<?php echo $swatch_term->pf_image_src(); ?>"  width="<?php echo $swatch_term->pf_get_width(); ?>px" height="<?php echo $swatch_term->pf_get_height(); ?>px" />
				</div>
				<div style="line-height:60px;">
					<input type="hidden" id="pf_<?php echo $this->pf_meta; ?>" name="pf_meta[<?php echo $this->pf_meta; ?>][photo]" value="<?php echo $swatch_term->pf_image_id(); ?>" />
					<button type="submit" class="upload_image_button button"><?php _e('Upload/Add image', 'woocommerce'); ?></button>
					<button type="submit" class="remove_image_button button"><?php _e('Remove image', 'woocommerce'); ?></button>
				</div>
				<script type="text/javascript">
	...
	public function pf_attribute_column($columns, $column, $id) {
		if ($column == $this->pf_meta) :
			$swatch_term = new PF_Attribute_Image($this->pf_meta, $id, $this->taxonomy, false, $this->image_size);
			$columns .= $swatch_term->pf_get_output();
		endif;
		return $columns;
	}
...
class PF_Attribute_Image {
	...
	public function pf_get_output($placeholder = true, $placeholder_src = 'default') {
		...
		if ( isset($this->thumbnail_src)) {
			...
			$picker .= '<img src="' . apply_filters('woocommerce_swatches_get_swatch_image', $this->thumbnail_src, $this->term_slug, $this->taxonomy_slug, $this) . '" alt="' . $image_alt . '" class="wp-post-image swatch-photo' . $this->pf_meta() . ' ' . $image_class . '" width="32" height="32"/>';
			$picker .= '</a>';
		} elseif ($placeholder) {
			...
			$picker .= '<img src="' . $src . '" alt="' . $image_alt . '" class="wp-post-image swatch-photo' . $this->pf_meta() . ' ' . $image_class . '" width="' . $this->width . '" height="' . $this->height . '"/>';
			$picker .= '</a>';
		} else {
			return '';
		}
